[ Conformance suite: fib. The first twelve Fibonacci numbers, one per line.
  Hand-written, not compiled from Turpentine.

  Tape layout: cell0 rounds left, cell1 a, cell2 b, cell3 the running sum,
  cell4 onward the decimal printer. The largest number printed is 89, so
  one byte per accumulator is enough and no multi-byte arithmetic appears.
  The printer takes a value 0 to 255 in its first cell, prints it without
  leading zeros and leaves its block at zero; it was checked against every
  value from 0 to 255 before it was used. This header runs in no loop:
  cell 0 is zero at the start, so the bracket pair is never entered. ]

++++++++++++>>+<<      twelve rounds; a = 0 and b = 1

[
  >[->>+>+<<<]>>[-<<+>>]>   a into the printer slot and a spare, then restore a
  ...
  ++++++++++.[-]       a newline
  <<<                  back to a
  [->>+<<]             a into the sum, emptying a
  >[-<+>>+<]           b into a and into the sum, emptying b
  >[-<+>]              the sum becomes the new b
  <<<-                 one round fewer
]
