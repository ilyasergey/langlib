...
compiler, same input encoding, same output decoding, same forward and divergence
proofs (assembled in the public `Ook` module). What is genuinely new here is the **syntactic** half, the part
...
`Ook.Divergence` transfers Brainfuck's divergence theorem for the other field.
