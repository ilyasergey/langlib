...
The geometric proof below lowers these traces to a codel grid and proves,
through `computeBlocks` and `evalGrid`, that DP/CC movement follows them,
including branches and back edges. `Piet.Divergence` proves positive progress
through the dispatcher on divergent inputs; the public `Piet` module
assembles both proof fields into `pietComplete`.
-/
