...
The image constructor `imageOf` below can choose a blank background and
establish `RegMem` for zero registers (`imageOf_regMem_init`). That is a
raw-image result, not a source initializer or a loadability proof.
