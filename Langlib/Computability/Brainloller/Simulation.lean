...
same forward and divergence proofs, assembled in the public `Brainloller` module.
...
bytes of output. `Brainloller.Divergence` transfers Brainfuck's divergence
proof for that decoded-program interface. Not proved, and not claimed: that Brainloller computes
every partial computable function (that step is the cited classical
result, Shepherdson and Sturgis 1963; see
`Langlib.Common.computes_of_turingComplete`), and the pixel walk above.
-/
