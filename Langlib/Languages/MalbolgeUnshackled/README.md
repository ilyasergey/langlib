...
`rotation-loop.mu`, and `marker-cycle.mu`, which never halt and want a `--fuel` bound. `--rot-width N` runs any of them at a width
...
(run with `lake test` from the repository root), in twelve suites: the small
...
inline), nine examples re-run at rotation width 37, strict loading, and the
...
The five runtime examples are derived files. Regenerate with
