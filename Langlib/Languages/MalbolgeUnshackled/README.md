...
| `marker-cycle.mu` | initializes and repeats rotation/reset of one marker in a 50-step cycle; use `--fuel 526`, no input; permissive loading | LangLib original, generated by `scripts/gen-mu-runtime.py` |
| `grow-loop.mu` | reuses one marker through successive width doublings; use `--fuel 1592` for initialization and three cycles, no input; permissive loading | LangLib original, generated by `scripts/gen-mu-runtime.py` |
| `bit-branch.mu` | initializes a reusable bit branch and takes paths 0, 1, 1, 0 before halting; use `--fuel 21`, no input; permissive loading | LangLib original, generated by `scripts/gen-mu-runtime.py` |
...
`rotation-loop.mu`, `marker-cycle.mu`, and `grow-loop.mu`, which never halt and want a `--fuel` bound. `--rot-width N` runs any of them at a width
...
(run with `lake test` from the repository root), in sixteen suites: the small
...
inline), eleven examples re-run at rotation width 37, strict loading, and the
...
Two further suites check a repeating rotation/reset cycle through nine
iterations at widths 16 and 37, including its no-op phases, restored rotor
and router, constants, return records and untouched input.

Two growing-marker suites inspect three consecutive doublings with the same
marker. The default setup reaches 18, then grows to 36, 72 and 144; the
odd-width run grows from 37 to 74, 148 and 296. Both connecting routes,
marker reset, code phases, constants and return records are observed.

Two bit-branch suites inspect initialization and four conditional jumps,
covering both outcomes in both phases of the no-op at address 1. They check
unchanged flags and continuation records, no input consumption, and the
exact halt boundary at widths 10 and 37.

The seven runtime examples are derived files. Regenerate with
