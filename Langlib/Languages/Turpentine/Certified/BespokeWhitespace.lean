...
/-- A related environment is well typed on the names in scope, which is what
the shared `evalExpr_hasTy` asks for. -/
theorem wellTyped_of {ctx : Frame} {ns : List String} {env : Std.HashMap String Value}
    {heap : Std.HashMap Int Int} (hcov : Covers ctx ns) (hag : Agrees ctx env heap) :
    ∀ x ∈ ns, ∀ (t : Ty) (v : Value), ctx.types[x]? = some t → env[x]? = some v →
      valHasTy v t = true := by
  intro x hx t v ht hv
  obtain ⟨a, ha⟩ := hcov x hx
  exact (hag x a ha v hv).2 t ht
      ...
      | assign x e =>
        ...
        | ok w =>
          ...
          · rw [← hσ]
            ...
            subst hu
            exact evalExpr_hasTy (wellTyped_of hcov hag) e tx w hoke hte hv
      | ite cnd t f iht ihf =>
        intro hok c code c' hem s hcode hlab σ σ' hex hag
        have hokc : okExpr ns cnd = true := by
          revert hok; simp only [okStmt, Bool.and_eq_true]; tauto
        ...
        | ok w =>
          ...
          · -- `outnum`, then the newline if this was `println`
            ...
            have hwt : valHasTy w Ty.int = true :=
              evalExpr_hasTy (wellTyped_of hcov hag) e Ty.int w hoke hint hv
            cases w with
          ...
          · -- the `true`/`false` branch, then the newline
            ...
            have hwt : valHasTy w Ty.bool = true :=
              evalExpr_hasTy (wellTyped_of hcov hag) e Ty.bool w hoke hbool hv
            cases w with
