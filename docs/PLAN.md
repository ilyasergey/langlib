...
| malbolge-unshackled | complete | **language landed**; the proof is **started, not finished** (`Langlib/Computability/MalbolgeUnshackled.lean`, axiom-clean, no `TuringComplete` witness yet). Proved: the `ProgLang` instance; the arithmetic of natural addresses; a step-level reading of `exec`; both obstructions (`decode_encrypt_ne`, `restTable_not_printable`); that **`jmp` is the only instruction that does not overwrite its own cell** (`jmp_cell_stable`), which is what lets anything loop; the jump-table spacing law (`gap_of_repeated_word`: a repeated entry's spacing is a difference of two opcodes, so 2 is impossible); and **a program that provably never halts** (`Langlib/Examples/MalbolgeUnshackled/loop.mu`, `Loop.neverHalts`), a three-step cycle turning on the fact that a cell both executed and jumped onto advances two orbit steps per pass. Measured, not proved: `cat.mu`'s period is 3060 = `lcm 68 9 6 5 4 2`, and that `loop.mu`'s prologue reaches the cycle. The rotation width is neutralised by not using `*` at all. See `docs/computability-malbolge-unshackled.md`. |
