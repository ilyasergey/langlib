...
self-encryption never bites. Two compiled programs are checked in under
...
**Compiled, not written** (`compiled/primes.mu`, `compiled/sort.mu`) — 348
and 268 characters, printing the primes up to 30 and six numbers in order.
Every other program on this page was written or searched for by a person.
These two are output: `turpentine compile --to malbolge-unshackled` applied
to `primes-mu.turp` and `sort-mu.turp`, the input-free twins of two of the
front end's examples.

They are the first programs here that need something the language did not
intend to offer. Malbolge's loader checks a character in 33..126 against its
...
constant in one cell. Two consequences: `--strict` refuses to load either
program, by design; and neither is confined to the alphabet a printable-only
...
Both are derived files, regenerated by `scripts/gen-mu-examples.sh` and
