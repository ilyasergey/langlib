...
The [JavaGen compiler regressions](../Langlib/Tests/JavaGenCompiler.lean)
exercise counter emission, zero/nonzero and nested loops, source round trips,
all four URM instructions, input-dependent answers and finite self-jump
prefixes. Expected answers are constants independent of the target decoder;
no source evaluation occurs during generation. The source fixtures under
`Langlib/Examples/JavaGen/compiled/` and `grow.jgen` are generated by
`scripts/gen-javagen-compiler-examples.lean`; its `--check` mode detects drift.
The real-Java suite additionally checks acceptance of the generated
